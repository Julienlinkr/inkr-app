...
router.post('/:id/acompte', requireAuth, async (req, res) => {
  try {
    ...
    // Récupérer le compte Stripe Connect de l'artiste
    const artistStripe = db.prepare('SELECT stripe_connect_id FROM users WHERE id = ?').get(req.user.userId);
    const connectedAccountId = artistStripe?.stripe_connect_id || null;
    ...
    // Si l'artiste a connecté son Stripe → l'argent va directement chez lui
